...
class UserActionService:
    ...
    def create_action(self, user_id: int, product_id: int, action_type: str) -> UserAction:
        action = UserAction(
            user_id=user_id,
    ...
    def delete_favorite(self, user_id: int, product_id: int) -> None:
        action = self.db.query(UserAction).filter(
    ...
    def get_user_favorites(self, user_id: int) -> List[UserAction]:
        return self.db.query(UserAction).filter(UserAction.user_id == user_id, UserAction.action_type == 'favorite').all()

    def get_user_view_history(self, user_id: int) -> List[UserAction]:
        return self.db.query(UserAction).filter(
            UserAction.user_id == user_id,
            UserAction.action_type == 'view'
